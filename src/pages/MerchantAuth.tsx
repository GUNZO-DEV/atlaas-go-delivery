...
import { Store, ArrowLeft, TrendingUp, BarChart3, Users } from "lucide-react";
...
const MerchantAuth = () => {
  ...
  const checkExistingAuth = async () => {
    ...
    if (user) {
      ...
      if (roles?.some(r => r.role === 'merchant')) navigate("/lyn-dashboard");
    }
  };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: { data: { full_name: validatedData.fullName }, emailRedirectTo: `${window.location.origin}/lyn-dashboard` },
      });
      if (error) throw error;
      if (data.user) {
        const { error: roleError } = await supabase.rpc("assign_merchant_role", { user_id_param: data.user.id });
        if (roleError) throw roleError;
      }
      toast({ title: "Check your email!", description: "We've sent you a verification link to activate your restaurant account." });
    } catch (error: any) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!roles?.some(r => r.role === 'merchant')) {
        ...
        throw new Error("This account is not registered as a restaurant.");
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-primary/5 to-accent/5 zellij-pattern relative overflow-hidden">
      <div className="absolute top-20 -right-32 w-72 h-72 bg-primary/8 rounded-full blur-3xl" />
      <div className="absolute bottom-20 -left-32 w-72 h-72 bg-accent/8 rounded-full blur-3xl" />
              ...
              <TabsContent value="signin" key="signin">
                <motion.form onSubmit={handleSignIn} className="space-y-4" variants={formVariants} initial="hidden" animate="visible" exit="exit">
                  <div className="space-y-2">
                    ...
                    <Input id="signin-email" type="email" placeholder="restaurant@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="h-11 transition-all focus:scale-[1.01]" />
                  </div>
                  <div className="space-y-2">
              ...
              <TabsContent value="signup" key="signup">
                <motion.form onSubmit={handleSignUp} className="space-y-4" variants={formVariants} initial="hidden" animate="visible" exit="exit">
                  <div className="space-y-2">
                    ...
                    <Input id="signup-name" type="text" placeholder="Atlas Tajine House" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="h-11 transition-all focus:scale-[1.01]" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="signup-email">Email</Label>
                    <Input id="signup-email" type="email" placeholder="restaurant@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="h-11 transition-all focus:scale-[1.01]" />
                  </div>
                  <div className="space-y-2">
